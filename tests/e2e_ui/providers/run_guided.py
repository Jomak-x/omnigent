"""Run the guarded real-PTY regression with one explicitly reviewed tmux binary."""
...
def main() -> None:
    ...
    if not args.tmux.is_absolute() or not args.tmux.is_file():
        parser.error("--tmux must be the absolute path of a reviewed tmux executable")
    ...
    source = Path(__file__).with_name("test_provider_guided.py")
    spec = importlib.util.spec_from_file_location("provider_guided_test", source)
